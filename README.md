...
```php
Simexis\Mongodb\MongodbServiceProvider::class,
```
...
```php
$app->register(Simexis\Mongodb\MongodbServiceProvider::class);
...
{
    return new Simexis\Mongodb\Connection($config);
});
```
...
Please change all `Simexis\Mongodb\Model` references to `Simexis\Mongodb\Eloquent\Model` either at the top of your model files, or your registered alias.

```php
use Simexis\Mongodb\Eloquent\Model as Eloquent;

class User extends Eloquent {}
```

If you are using hybrid relations, your MySQL classes should now extend the original Eloquent model class `Illuminate\Database\Eloquent\Model` instead of the removed `Simexis\Eloquent\Model`. Instead use the new `Simexis\Mongodb\Eloquent\HybridRelations` trait. This should make things more clear as there is only one single model class in this package.

```php
use Simexis\Mongodb\Eloquent\HybridRelations;
...
```php
'Moloquent'       => Simexis\Mongodb\Eloquent\Model::class,
```
...
```php
'Simexis\Mongodb\Auth\PasswordResetServiceProvider',
```
...
```php
Simexis\Mongodb\MongodbQueueServiceProvider::class,
```
...
```php
use Simexis\Mongodb\Eloquent\SoftDeletes;
...
If you're using a hybrid MongoDB and SQL setup, you're in luck! The model will automatically return a MongoDB- or SQL-relation based on the type of the related model. Of course, if you want this functionality to work both ways, your SQL-models will need use the `Simexis\Mongodb\Eloquent\HybridRelations` trait. Note that this functionality only works for hasOne, hasMany and belongsTo relations.
